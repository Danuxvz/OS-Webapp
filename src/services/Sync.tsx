...
export async function pullCharactersExport() {
	...
	const CARD_IDS = [
		"AE_Card","Basic_Attack","Ethrielle","Engaar","Halagar","Interpretar",
		"Intimidar","Negociar","Persuadir","Rogar","Seducir","Sobornar"
	];
	const CONSUMABLE_IDS = [
		"KudagiBento","AstralDoguBento","GetStrongBento",
		"ScarletSpectralMiso","ShellSushi","SpicyFireRamen",
		"MomijiManju","MochisDeBaku","TaiyakiKijyo"
	];
	...
	for (const exp of exports) {
		...
		console.info(`pullCharactersExport: synced character "${localChar!.charName}" with export "${exp.name}"`);
	}
}
	...
	for (const tableName of candidates) {
		try {
			...
			if (error) {
				...
				console.warn(`fetchOldUserRow: no data from ${tableName}:`, error.message ?? error);
				continue;
			}
...
async function migrateOldUserDataIfNeeded() {
	...
	if (!oldRow) {
		console.info("migrateOldUserDataIfNeeded: no old user row found, skipping migration");
		return;
	}
	...
	let raw = oldRow.data;
	if (raw == null) {
		console.info("migrateOldUserDataIfNeeded: old data is null/undefined, skipping");
		return;
	}
	...
	if (typeof raw === "object") {
		...
		try {
			...
			console.warn("migrateOldUserDataIfNeeded: failed to parse old data after cleaning. Sample start:", s.slice(0, 200));
		...
		console.warn("migrateOldUserDataIfNeeded: unexpected old data type:", typeof raw);
		return;
	}
	...
	if (localCharacters.length === 0) {
		console.info("migrateOldUserDataIfNeeded: no local characters to merge into, skipping");
	...
	if (localCharacters.length > 1 && parsed.name) {
		const match = localCharacters.find(c => c.charName.toLowerCase() === String(parsed.name).toLowerCase());
